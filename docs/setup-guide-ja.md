...
すると:
- ブラウザが自動で開きます
- `http://localhost:5000` というページが表示されます
- これが LinkStash の画面です！

> ブラウザが自動で開かない場合は、手動で `http://localhost:5000` にアクセスしてください。
...
> **ヒント**: Windows の場合、LinkStash フォルダ内に以下の内容で `起動.bat` というファイルを作ると、ダブルクリックで起動できます:
> ```
> python app.py
> pause
> ```
